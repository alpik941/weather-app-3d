...
export default function WeatherScene({ 
	...
	if (mode === 'css') {
		return (
			<div className="relative w-full h-48 flex items-center justify-center overflow-hidden">
				...
				{/* Cloud overlay for rainy CSS mode */}
				{isRaining && (
					<div className="absolute inset-0">
						{Array.from({ length: 10 }).map((_, i) => (
							<span
								key={i}
								className="absolute rounded-full bg-white/40 dark:bg-white/20 blur-xl"
								style={{
	...
	return (
		...
		>
			...
			{/* Stars only when not raining */}
			{isNight && !isRaining && <Stars radius={100} depth={50} count={5000} factor={4} saturation={0} fade />}

			{/* Dynamic Sky on clear daytime with realistic sun path */}
			{!isNight && !isCloudy && !isRaining && (
				<DynamicSky
					sunrise={typeof sunrise === 'number' ? sunrise : undefined}
					sunset={typeof sunset === 'number' ? sunset : undefined}
					currentTime={typeof currentTime === 'number' ? currentTime : Date.now()}
					weather={weather}
				/>
			)}

			{/* Celestial Bodies: show Moon on clear nights; suppress Sun when DynamicSky is active */}
			{!isRaining && !isCloudy && isNight && (
			...
			{(isCloudy || isThunderstorm || isRaining) && (
				<DynamicClouds coverage={isRaining ? Math.max(0.7, cloudCoverage) : cloudCoverage} isStormy={isThunderstorm} />
			)}
